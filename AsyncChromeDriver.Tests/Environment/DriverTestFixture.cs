...
namespace Zu.AsyncChromeDriver.Tests
{
    public abstract class DriverTestFixture
    {
        ...
        protected Task<bool> WaitFor(Func<Task<bool>> waitFunction, string timeoutMessage)
        {
            return WaitFor<bool>(waitFunction, timeoutMessage);
        }

        protected Task<T> WaitFor<T>(Task<T> waitFunction, string timeoutMessage)
        {
            return this.WaitFor<T>(() => waitFunction, TimeSpan.FromSeconds(5), timeoutMessage);
        }

        protected Task<T> WaitFor<T>(Func<Task<T>> waitFunction, string timeoutMessage)
        {
            return this.WaitFor<T>(waitFunction, TimeSpan.FromSeconds(5), timeoutMessage);
        }

        protected async Task<T> WaitFor<T>(Func<Task<T>> waitFunction, TimeSpan timeout, string timeoutMessage)
        {
            DateTime endTime = DateTime.Now.Add(timeout);
            T value = default(T);
            Exception lastException = null;
        ...
        //protected async Task<bool> WaitFor(Task<string> waitFunction, string expected, string timeoutMessage)
        //{
        //    await waitFunction;
        //    return WaitFor<bool>(waitFunction, expected, timeoutMessage);
        //}

        protected Task<T> WaitFor<T>(Task<T> waitFunction, T expected, string timeoutMessage) where T : class
        {
            return this.WaitFor<T>(waitFunction, expected, TimeSpan.FromSeconds(5), timeoutMessage);
        }

        protected async Task<T> WaitFor<T>(Task<T> waitFunction, T expected, TimeSpan timeout, string timeoutMessage) where T : class 
        {
            DateTime endTime = DateTime.Now.Add(timeout);
            T value = default(T);
            Exception lastException = null;
